...
if __name__=="__main__":
    ...
    label_index = Con3Numpy('AGCN_TRdataset_M','GCN_TRdataset')#替换为对应文件,修改'AGCN_TRdataset_M'

    label_index1 = label_index[0:200]

    label_index2 =[]
    for i in range(label_index1.shape[0]):
        sss = np.argwhere(Data[img_label]==label_index1[i])
        label_index2 = np.append(label_index2,sss[0,0])
    labeled_set= np.array(label_index2, dtype='int')
    ...
    for cycle in range(CYCLE):
        ...
        lbl = np.arange(SUBSET, SUBSET+cycle* AddENDUM + intital_labelnum, 1)
        ...
        correct_labeled = (preds[SUBSET:, 0] == labels[SUBSET:, 0]).sum().item() / (cycle  * AddENDUM + intital_labelnum)
        correct_unlabeled = (preds[:SUBSET, 0] == labels[:SUBSET, 0]).sum().item() / SUBSET
        correct = (preds[:, 0] == labels[:, 0]).sum().item() / (SUBSET + cycle * AddENDUM + intital_labelnum)
    ...
    ucx=Data[img_gt][labeled_set,:]

    GCN_dataset=np.array(labeled_set2)
    GCN_dataset2=np.array(unlabeled_set2)
    sio.savemat("data/AGCN_TRdataset_M_IP100.mat", {'GCN_TRdataset':GCN_dataset})
    sio.savemat("data/AGCN_TEdataset_M_IP100.mat", {'GCN_TEdataset':GCN_dataset2})