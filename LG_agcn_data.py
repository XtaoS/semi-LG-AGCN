...
if __name__=="__main__":
    ...
    label_index = Con3Numpy('AGCN_TRdataset_M_600','GCN_TRdataset')#替换为对应文件,修改'AGCN_TRdataset_M'
    ...
    for cycle in range(CYCLE):
        ...
        correct_labeled = (preds[SUBSET:, 0] == labels[SUBSET:, 0]).sum().item() / ((cycle + 1) * AddENDUM + + intital_labelnum)
        correct_unlabeled = (preds[:SUBSET, 0] == labels[:SUBSET, 0]).sum().item() / SUBSET
        correct = (preds[:, 0] == labels[:, 0]).sum().item() / (SUBSET + (cycle + 1) * AddENDUM + intital_labelnum)
    ...
    sio.savemat("data/AGCN_TRdataset_100.mat", {'GCN_TRdataset': GCN_dataset})
    sio.savemat("data/AGCN_TEdataset_100.mat", {'GCN_TEdataset': GCN_dataset2})
